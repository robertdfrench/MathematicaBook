...
In Cell 3, nothing interest happens. We note that \expr{i} and \expr{j} are both \expr{11} like we expected them to be. But now let's evaluate Cell 4 and see what happens. The output from Cell 4 tells us that \expr{j} is still \expr{11}, but that \expr{k} has been updated to reflect the new value of \expr{i}. This is because the definition we gave for \expr{k} in Cell 2 is re-evaluated \emph{every time} we use \expr{k} in an expression.

If we evaluate Cell 5, we see the same thing has happened: \expr{j} is still \expr{11}, but \expr{k} is now \expr{30}, reflecting the fact that we changed the value of \expr{i} again.
...
Evaluating the above cell will give you an output cell containing the numebr 1. This is an example of invoking the \expr{Cos} function directly for a single argument. If we want to evaluate \expr{Cos} for a list of arguments, we can instead treat it as a variable and pass it as an argument to the function \expr{Map}:
...
Evaluate this and you will see an output cell containing $\{1,1,1\}$.\footnote{It is worth your while to play around with this code until it ``clicks'' for you} Of course, not all functions are listable, and sometimes using this feature can make your code harder to read and thus debug. That is why \expr{Map} is a trustworthy alternative. 

{\bf Exercises}. 
\begin{enumerate}
	   \item Explain why the code in Cell 2 produces only the single output of \expr{11}
	   \item Explain why Cell 6 produces \expr{\{11,x + 10\}} as an output.
